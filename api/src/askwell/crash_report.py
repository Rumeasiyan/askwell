...
def install_loop_handler(settings: Settings, component: str) -> None:
    """Write a report for an exception the running event loop reports as
    never retrieved — a background task nobody awaited (issue #715) — then
    let the previous handler, or the default one, log it as before.

    Only `context["exception"]` is read. `context["message"]` is free text
    and can quote anything, so it stays out for the same reason the
    ...
    def handler(current: asyncio.AbstractEventLoop, context: dict[str, Any]) -> None:
        ...
        if isinstance(error, Exception):
            write(error, component=component, settings=settings)
...
def report_task_failure(settings: Settings, component: str) -> Callable[[asyncio.Task[Any]], None]:
    """A done-callback that writes a report when a *kept* task fails.

    A task stored and awaited at shutdown counts as retrieved, so the loop
    handler above never sees its exception; without this, a failure at
    startup would leave no report until the process stopped, if then.
