...
def test_download_refuses_anything_but_a_report_name(crash_settings: Settings, name: str) -> None:
    ...
    response = client.get(f"/crash-reports/{name}")

    # An encoded traversal decodes to a path this route does not match, so it
    # falls through to the interface catch-all, whose answer depends on whether
    # the frontend is built (404 locally, 503 in CI). What must hold either way
    # is that nothing outside a report name is ever served.
...
async def _fail_in_background() -> None:
    question = QUESTION  # noqa: F841 — a local variable a report must not read
    raise RuntimeError(f"model discovery failed on {FILENAME} for {QUESTION!r}")


async def test_a_task_nobody_awaits_still_leaves_a_report(crash_settings: Settings) -> None:
    # Issue #715: a background task's exception reaches neither the request
    # handler nor `sys.excepthook`, only the loop's exception handler.
    loop = asyncio.get_running_loop()
    seen: list[str] = []
    loop.set_exception_handler(lambda _loop, context: seen.append(context["message"]))
    try:
        crash_report.install_loop_handler(crash_settings, "api")
        task = loop.create_task(_fail_in_background())
        await asyncio.wait({task})
        loop.call_exception_handler(
            {
                "message": f"Task exception was never retrieved: {QUESTION}",
    ...
    assert len(seen) == 1, "the previous handler still runs"
    report = _only_report(crash_settings.crash_report_dir)
    assert "builtins.RuntimeError" in report
    for secret in SECRETS_IN_PLAY:
        assert secret not in report, secret


async def test_a_kept_task_reports_when_it_fails_not_at_shutdown(crash_settings: Settings) -> None:
    # The lifespan keeps its tasks and awaits them after `cancel()`, which
    # counts as retrieving the exception, so they report through a callback.
    failing = asyncio.create_task(_fail_in_background())
    failing.add_done_callback(crash_report.report_task_failure(crash_settings, "api"))
    await asyncio.wait({failing})
    ...
    cancelled.add_done_callback(crash_report.report_task_failure(crash_settings, "api"))
    cancelled.cancel()
    await asyncio.wait({cancelled})
    await asyncio.sleep(0)

    assert len(list(crash_settings.crash_report_dir.iterdir())) == 1, "cancellation is not a crash"
