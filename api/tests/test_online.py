...
async def test_nothing_is_authorised_when_no_destination_is_configured(
    session: AsyncSession, settings: Settings, redis_store: dict[str, str]
) -> None:
    conversation_id = await _conversation(session)
    unconfigured = settings.model_copy(update={"online_ai_destination": None})

    with pytest.raises(online.OnlineUnavailable):
        await online.enable(session, unconfigured, conversation_id)
    await session.commit()
