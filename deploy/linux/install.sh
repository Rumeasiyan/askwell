...
#     web/src-tauri/target/release/askwell-shell                      — the desktop shell binary (M7-TAURI-DEPLOY-181/183)
#
...
# scope (see docs/decisions.md, this date). The one artefact this script
# cannot produce itself is the compiled shell binary: building it needs the
# Rust toolchain, which a non-technical installing user must never be asked
# for, so its absence is a refusal naming the missing file and the build
# command that produces it — never a silent fall back to a browser tab, which
# is exactly the thing M7-TAURI-DEPLOY-181 exists to make unnecessary.
#
...
place_files() {
  ...
  [ -f "$INSTALL_PREFIX/.env" ] || cp "$REPO_ROOT/.env.example" "$INSTALL_PREFIX/.env"
