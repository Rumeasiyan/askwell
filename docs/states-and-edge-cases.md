...
| **Duplicate (same sha256)** | Named as already present, linked to the existing document, **with both paths** — the one indexed and the one skipped. Not re-ingested | `documents.sha256` exists in `architecture.md` §7 for this. Which copy answers cite is the whole question; "already present" without a path sends someone through their own filing to find out what happened |
| **A zero-byte file** | Rejected by name, with the fact that it is empty and that whatever wrote it did not finish | It would produce no chunks and sit permanently "ready" while answering nothing — and every empty file shares one hash, so the second would be reported as a duplicate of the first, which is true of the bytes and misleading about the documents |
| **A file still being written** (a download, a sync client, a save in progress) | Named, with the fact that it was not indexed and that adding it again once it settles will work | Hashed, found to have changed under the read, re-hashed, and refused after three attempts. A hash spanning two versions of a file identifies neither, and every later duplicate check against that row would be wrong — silently, and for as long as the row survives |
