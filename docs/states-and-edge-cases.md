...
| **About — clicking the source or issue-tracker link while offline** | Normal browser failure-to-load for that tab; the rest of Askwell is unaffected | `M7-DOC-DOC-163`. A user-initiated `<a href>` away from the app is not the automatic reach C1 forbids — `web/scripts/check-offline.mjs` treats it as inert, reported but never a failure |
| **About — report a problem while offline** | The support boundary and the security policy still open: both are bundled into the build (`web/public/support.txt`, `web/public/security-policy.txt`), so what is answered and what a report needs can be read with no network. Only the final "Open an issue" link needs one | `M7-DOC-DOC-164`. Someone offline is often exactly the person with a problem; the part that sets their expectation must not depend on the network the product promises not to need |
...
| **About — update checking** | Not yet built. `docs/ux/settings.md` §7's opt-in weekly check has a backend (`M7-UPDATE-BE-161`) but no settings-screen control yet | Same "visible and honest, not hidden" pattern as export and prune above |
