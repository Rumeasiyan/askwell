...
| **Export and prune, wiring the settings screen to the real endpoints** | Not yet built — `M7-LOG-BE-154`/`M7-LOG-BE-155` are both real now; the settings-screen entry point that calls them is a separate, not-yet-started frontend ticket | Same pattern `settings.md` §3 already uses for Online AI ahead of Phase 7 — visible and honest rather than hidden or silently broken |
