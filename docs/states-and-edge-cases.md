...
| **Retention window changed** | Confirmed | `M7-SET-FE-148`/`M7-LOG-BE-154`. The value is a real decisions record and now has a real prune behind it (`POST /log-prune`) |
| **Retention window shorter than the age of the oldest interaction on record** | Refused (`400`, `confirmation_required: true`) unless the caller explicitly confirms | `M7-LOG-BE-154`'s own edge case — `askwell.log_budget.set_retention_months`. Nearly everything currently stored would become prunable; refusing by default and requiring an explicit `confirmed: true` matches the shape `log_export`'s `acknowledged_decrypted_export` already established for a consequence the caller must actively accept |
| **Prune attempted without a covering export** | Refused (`POST /log-prune` → `409`, `export_offered: true`) | `M7-LOG-BE-154`'s own edge case. Only a full-history export (`since` unset) can advance `askwell.retention.exported_through`; pruning unexported records destroys the user's own history |
| **Prune succeeds** | The interactions store shrinks; the decisions log gains a record naming the cutoff and how many records were removed; `askwell-verify` still passes, walking the interactions chain from the boundary that prune recorded rather than the universal genesis value | `M7-LOG-BE-154`. Decisions and memory are never touched |
| **Export and prune, backend built, no frontend wiring yet** | Storage settings still show a stated, disabled entry point | `M7-SET-FE-148`'s own copy. Both `POST /log-export` (`M7-LOG-BE-155`) and `POST /log-prune` (`M7-LOG-BE-154`) exist now, but no frontend ticket has wired the settings screen's buttons to them — same pattern `settings.md` §3 uses for Online AI ahead of Phase 7, now waiting on a frontend ticket rather than a backend one |
