...
| **Log verification, large log** | Progress shown; the run is interruptible mid-walk | `M7-LOG-FE-156`. A cancelled run reports neither intact nor broken — asserting either about records never checked would be its own false claim |
| **Log verification break at a prune boundary** | Not yet distinguishable from tampering — `M7-LOG-BE-154` (prune) does not exist, so no legitimate boundary can occur today | `M7-LOG-FE-156`, issue #516. Every break `verify()` can currently produce is real; the distinction is this ticket's own stated Edge Case and is deferred to whoever builds the prune itself |
...
| **Retention window changed** | Confirmed, with an explicit statement that nothing is pruned yet | `M7-SET-FE-148`. The value is a real decisions record; the prune that acts on it is `M7-LOG-BE-154`, not yet built. Implying enforcement that does not exist would be worse than the honest gap |
| **Export and prune, before `M7-LOG-BE-155`/`M7-LOG-BE-154` exist** | A stated, disabled entry point explaining it is not built yet | Same pattern `settings.md` §3 already uses for Online AI ahead of Phase 7 — visible and honest rather than hidden or silently broken |
