...
**Whichever path, take a backup first.** There is no backup button yet (issue #615), so it is four commands. Every route except `/health` needs the session cookie the interface issues, so the first command fetches one. Use `127.0.0.1` throughout: a cookie fetched from `127.0.0.1` is not sent to `localhost`.

```
curl -s -c askwell-cookie -o /dev/null -H "accept: text/html" http://127.0.0.1:8000/
curl -s -b askwell-cookie -X POST http://127.0.0.1:8000/backup          # note the "id"
curl -s -b askwell-cookie http://127.0.0.1:8000/backup/<id>             # repeat until "status": "done"
curl -s -b askwell-cookie -o askwell-backup.zip http://127.0.0.1:8000/backup/<id>/download
```

Keep `askwell-backup.zip` outside the install prefix. §1.2 deletes the volumes, and a backup left on them goes too. A backup made by the newer version **cannot** be restored into the older one: `askwell.restore._check_version` refuses a backup newer than the install, by name, before touching a row. It is still the only way to get back to where you started if the rollback itself goes wrong.
...
4. **Confirm.** `curl -s http://127.0.0.1:8000/health` reports the older version, and every component is reachable. Ask a question you know the answer to, and check that it cites the right page.

5. **Dismiss the "Newer version" line.** If update checks are on, Settings → About still shows the broken release as a newer version, and pointing the person straight back at it is the one thing this procedure must not do. Choose **Dismiss until a newer one**. The line returns only when a version above the broken one is found, which is the fixed release (§4.3).

If the older release is `0.7.34` or later, its first start appends one `version_changed` record to the decisions log, from the newer version to the older one (`askwell.update_check.record_running_version`). That is the rollback being recorded, not an error. The chain still verifies.
...
4. **Restore.** There is no restore screen yet (#615). Restore reads the backup from a path **inside the api container**, so copy it onto the state volume first. Not `/tmp`: in the container that can be the read-only folders mount (`ASKWELL_ROOTS_MOUNT`), and the copy fails.
   ...
   ```
   podman compose cp askwell-backup.zip api:/var/lib/askwell/askwell-backup.zip
   curl -s -c askwell-cookie -o /dev/null -H "accept: text/html" http://127.0.0.1:8000/
   curl -s -b askwell-cookie -X POST http://127.0.0.1:8000/restore/inspect \
     -H 'content-type: application/json' -d '{"path": "/var/lib/askwell/askwell-backup.zip"}'
   curl -s -b askwell-cookie -X POST http://127.0.0.1:8000/restore \
     -H 'content-type: application/json' -d '{"path": "/var/lib/askwell/askwell-backup.zip"}'
   curl -s -b askwell-cookie http://127.0.0.1:8000/restore/<id>   # repeat until "status": "done"
   ```

   Inspect states the version, chunk count and re-embedding time before anything is written. A backup that is not a readable Askwell backup is refused with `422` and the reason. A non-empty database is refused with `409`: go back to step 1. A passphrase-protected backup needs `"passphrase"` in the `POST /restore` body. More detail: `docs/manual-tests/M7-BACKUP-BE-158.md`.

5. **Confirm** as in §1.1 steps 4 and 5, then run `podman compose exec api askwell-verify`. Both audit chains must verify.
...
- A background task fails with nothing waiting on its result: the API's startup model reapply and model discovery, or any task the event loop reports as never retrieved (`install_loop_handler`, `report_task_failure`). A failure at startup is the shape a platform-specific release bug often takes, so it must not be the one case that leaves nothing to attach.
...
| `askwell_version`, `component`, `occurred_at` | `0.7.35`, `api`, ISO 8601 UTC |
