...
## 2026-08-30 — A user-origin memory/schema-note write retires the active inference for the same subject; "learned from a deleted source" is a join against the soft-deleted source row, not a snapshot

**Decision:** `M3-STORE-BE-076`'s `askwell.memory` adds one rule the ticket text implies but does not state outright: when `write_memory_fact`/`write_schema_note` is called with a user-origin (`origin != 'inferred'`), it also marks `superseded_by` on any still-active `inferred` row for the same subject (memory) or table/column position (schema notes), in the same statement, with its own `memory_superseded`/`schema_note_superseded` decisions record. Without this, answering a clarification for a subject Askwell had already guessed at would leave two active rows — the guess and the answer — both surfacing at retrieval time with no way to tell the model "the second one replaced the first" beyond precedence ordering, which is a display convenience, not the same guarantee as `superseded_by`. The ticket's stated rule ("an inference never overwrites a user-supplied fact") only forbids the inference arriving *after*; it says nothing about the reverse direction, so this closes the gap by symmetry rather than by an explicit line in the ticket. Separately, `memory.source_id` was added (nullable, `ON DELETE SET NULL`, migration `2ae457a0587a`) so a fact can name the source it came from, and "labelled as learned from a deleted source" (the ticket's own testing note) is answered by joining to `sources.deleted_at` at read time rather than by snapshotting the source's name onto the fact — `askwell.sources.delete_source` soft-deletes (`status = 'deleted'`, row kept), so the source row supplying that name never actually disappears, and a live join stays correct if the source is ever renamed, where a snapshot would go stale silently.

**Why:** the alternative to auto-superseding was to leave both rows active and rely on `get_active_memory_facts`/`get_active_schema_notes`'s ordering (user before inferred) to make sure only the right one is *used* — rejected because "used first" and "superseded" are different guarantees: an ordering rule is a property of the query, re-derived every time and only as strong as every caller remembering to take `[0]`; `superseded_by` is a property of the data, correct for any caller including ones this ticket does not know about yet (`M3-APPLY-RET-078`'s retrieval, `M3-CORRECT-FE-081`'s chips). The alternative to the join for deleted-source labelling was a denormalised `source_label` text column captured at write time — rejected because sources here are never hard-deleted (checked directly: `askwell.sources.delete_source` issues `UPDATE ... SET status = 'deleted'`, never a row `DELETE`, so the `ON DELETE SET NULL` on `memory.source_id` is a guard for a path that cannot currently execute), which means a snapshot buys nothing a join does not already give for free, at the cost of a value that could drift from the source's actual current name.

**Consequences:** if a future ticket ever makes source deletion an actual row removal (e.g., for real disk reclamation), `memory.source_id` silently nulls via `ON DELETE SET NULL` and the "deleted from" label is lost — at that point a denormalised snapshot becomes the right call, and this decision should be revisited rather than assumed to still hold. `write_memory_fact`/`write_schema_note` now issue an extra `UPDATE` on every user-origin write to check for a superseded inference, which is a no-op scan when none exists (indexed on `subject`/`(source_id, table_name, column_name)` via the existing lookup pattern, not a new index) so the cost is negligible against write volumes this feature will ever see.

**Refs:** `AGENTS.md` §3 (none of C1–C10 touched); ticket `M3-STORE-BE-076`; `docs/memory-and-clarification.md` §3, §7, §8; `api/src/askwell/memory.py`; `api/src/askwell/db/migrations/versions/20260830_2ae457a0587a_memory_source_id.py`; `api/src/askwell/sources.py`'s `delete_source`.
