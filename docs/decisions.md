...
**Bar for an entry:** something a competent person would later ask *"why is it like this?"* about. Architecture changes, dependency choices, resolved `PRD.md` §11 questions, reversals. **Not** routine implementation choices — those are visible in the diff.
...
**Consequences:** Every unit of work costs an issue and a PR. The `blocked:decision` label is now the visible queue of `PRD.md` §11 items. If the tracker fills with noise the labels stop being read, so the "too small for an issue" carve-out in `AGENTS.md` §8 has to be honoured.

**Refs:** `AGENTS.md` §8, `PRD.md` §9, §11.
...
`0.1.0` rather than `1.0.0` because nothing is shippable; `PRD.md` is itself marked v0.1 draft. Per-change bumping rather than release-only was chosen so that a `BRAIN.md` entry, an issue's closing comment, and a version all name the same thing — which is what makes "what was in the pilot build?" answerable six months later.
...
**Refs:** `AGENTS.md` §7, `VERSION`, `CHANGELOG.md`, `PRD.md` §9.
...
**Why:** The PRD draft named `Qwen3.5 4B` and `Qwen3.6 27B` — neither is a real release, and 27B is a Gemma parameter count, not a Qwen one. Left in place, a deployer would have gone looking for a GGUF that does not exist, on an air-gapped install where they cannot simply search for the right name. Corrected to real models on the same family as the already-correct `standard` row, so all three profiles share one tokeniser and one prompt format — which matters because the eval suite's pass bars in `PRD.md` §7 are meant to be comparable across profiles.
...
**Refs:** `PRD.md` §5.3, §7; `AGENTS.md` §4; commit `8e1f21d`.
...
**Refs:** `PRD.md` §2, §1.1, §8; `BRAIN.md` decisions log.
...
**Refs:** `PRD.md` §4.1, §5.3, §7; `BRAIN.md` decisions log.
...
**Refs:** `PRD.md` §5.1, §5.2, §5.3; `BRAIN.md` decisions log.
...
**Refs:** `PRD.md` §5.1, §6; `BRAIN.md` decisions log.
...
**Refs:** `PRD.md` §5.1, §5.2; `AGENTS.md` §6; `BRAIN.md` decisions log.
