...
## 2026-08-28 — Partial and conflicting-sources UI states parse the streamed answer text client-side; a document date reuses `GET /documents/{id}` rather than growing the citation event

**Decision:** `M2-PARTIAL-FE-058`. The Ask screen tells a partial answer and a conflicting-sources answer apart from an ordinary one by mirroring `askwell.agent.partial.split_partial_answer` and `askwell.agent.conflict.split_conflict_answer` in TypeScript (`web/lib/answer-annotations.ts`), reading the "Not covered:" and "Conflicting sources on ...:" lines back out of `turn.answer` — the same full text `AnswerProse` already has, since those lines stay in the composed text rather than being stripped server-side. No new field was added to the `citation` SSE event or the `done` event for this. Separately, a conflicting answer's cards need a date and, if superseded since the answer streamed, that fact — neither exists anywhere in the wire protocol today, so `web/lib/document-dates.ts` fetches `GET /documents/{id}` per cited document (the same pattern `SupersededBanner`, `context-rail.tsx`, already uses for the same endpoint), and that endpoint gained one field, `added_at` (`api/src/askwell/documents.py`), reading a column that already existed on every row.

**Why:** `M2-PARTIAL-BE-057` and `-059` both compute `partial_coverage`/`uncovered_aspects`/`conflict_detected`/`conflict_topic` and write them to `messages.trace` and `audit_interactions` — server-side only. Neither is on the `done` SSE event, and no trace-fetch endpoint exists yet for the browser to read them from (`docs/ux/ask.md` §4's "How did you get this?" trace panel is unbuilt). Two options were weighed against building the state detection into this ticket regardless. First, add the four fields to the `done` event: rejected for now, because the exact same signal the browser would use them for — whether "Not covered:"/"Conflicting sources on ...:" lines are present — is already sitting in the answer text every browser already has, so a new wire field would duplicate data already in hand rather than add anything a parse cannot get, at the cost of a second implementation of the same split logic. If the trace panel ticket later needs `partial_coverage`/`conflict_detected` as booleans rather than reparsed text (for example, to render before the answer is fully known), that is exactly the moment to reconsider. Second, for the document date: growing the `citation` SSE event with `added_at`/`superseded_by`/`superseded_at` was considered and rejected because every ordinary citation would carry three fields only a conflict's own cards ever render, for data a single already-existing endpoint already returns — the same reasoning `context-rail.tsx`'s own comment gives for reusing this endpoint instead of growing the metadata endpoint's own join ("no banner endpoint built ahead of the screen that needed it," issue #141). `added_at` is the only date source that exists anywhere in this codebase — no ingestion-metadata or filename-derived date is extracted — so it is what backs the ticket's own fallback rule ("where neither exists, the added date is used and labelled as such") unconditionally rather than as a fallback branch.

**Consequences:** the client-side regex in `answer-annotations.ts` must be kept in lockstep with `partial.py`/`conflict.py`'s own fixed-line conventions by hand — a future change to either server-side line format silently breaks the FE's detection with no type error to catch it, the same risk `claims.ts` already carries for `segment_claims`. `GET /documents/{id}` is now on the hot path for every conflicting-sources card (one fetch per cited document, client-side), rather than delivered inline with the answer; acceptable at conflict-answer volume, would need revisiting if it becomes a per-citation cost on every ordinary answer. Reversing the document-date half means adding the three fields to the `citation` event instead and dropping the per-card fetch.

**Refs:** `docs/backlog/M2-it-says-when-it-doesnt-know.md` (`M2-PARTIAL-FE-058`), `web/lib/answer-annotations.ts`, `web/lib/document-dates.ts`, `web/components/ask/ask-screen.tsx`, `web/components/ask/provenance-margin.tsx`, `api/src/askwell/documents.py`, issue #225 (superseded-source edge case only reachable for a re-viewed past turn).
