...
**Version under test:** `0.7.33`. Run `cat VERSION` and update this line if it has moved on.
...
| `api/tests/test_release_checklist.py` | 18 tests that fail if the documents drift from the eval suites, the build plan or the screens |
...
> `curl localhost:8000/network`, which answers `No session.`, to Settings → Network activity.
...
1. In a terminal:
   ...
   **You should see:** the last line reads `18 passed, … deselected` with no `failed`. (Seen
   on `0.7.33`: `18 passed, 1921 deselected, 2 warnings`.)
...
4. Put the file back and run the tests once more:
   ...
   **You should see:** `18 passed` again.
...
4. **The gate table.** ☐
   **You should see:** twelve rows, G1 to G12, each with all five columns filled: Gate, Pass
   condition, How to run it, Evidence lives in. The gates are: version and changelog,
...
2. **W1.1.** Run `curl -s localhost:8000/health`.
   **You should see:** JSON starting `{"version":"0.7.33"`, and a `components` list in which
...
3. **W1.2.** Run `scripts/verify-localhost-binding.sh`.
   **You should see:** it reports only loopback addresses bound, and exits without an error.
...
22. **W4.3.** Add the two disagreeing PDFs the same way as step 12, wait for them to index,
...
25. **W5.1.** Click **Add a source**, choose the CSV with **Choose files**.
    **You should see:** it imports. Click **Clarifications** in the rail. A question about the
    ambiguous column is waiting.

26. **W5.2.** Back on **Add a source**, in the dump box click **Choose a dump file** and pick
    the `.sql` file.
...
28. **W5.4.** **Add a source** → **Connect a database**. Enter the host, port, database, user
...
45. **W9.1.** Click **Settings** and scroll through every section: Model and speed, Online AI,
    ...
    **You should see:** each one with real content. No lorem ipsum, no "TODO", no button that
    does nothing when pressed. The only disabled control is the Online AI switch.
...
47. **W9.3.** Under **About**.
    **You should see:** **Version** reads `0.7.33`. **Licence** reads Apache-2.0, with **Read
...
52. **W9.8.** In the terminal: `podman compose exec api askwell-verify`.
...
1. Copy the format block from `docs/release-log.md` into `/tmp/trial-release-entry.md`. Fill
   ...
   | G10 artefacts | `BLOCKED` | No installable bundle (#559); no clean macOS or Windows machine (#590, #592) |
...
- **Backup and restore have no Settings control** (#615). W9.4–W9.5 point at
  `docs/restore-release-test.md` until one exists.
