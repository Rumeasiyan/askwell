...
**Version under test:** `0.7.22` (check `cat VERSION` — bump this line if it has moved on).
...
**Note on issue #632, fixed this session, not separately exercised by this walkthrough:** the
alternatives list's own catalog check (`available_alternatives`) only ever lists sha256-verified
*shipped* weights, so the file swapped to in this step and step 10 (`fake.gguf`, a placed but
uncatalogued file) is correctly marked Unverified either way. The bug this fix addresses needs
a second real, full-size, catalog-matching shipped model file (e.g. both the `light` and
`standard` tier weights present at once) to exercise for real — not attempted here, since
neither is present on this build host; covered instead by `api/tests/test_model_select.py`'s
`test_a_swap_to_a_catalog_recognised_alternative_is_marked_shipped`, which drives
`select_user_model` directly with the catalog check mocked to a match.
