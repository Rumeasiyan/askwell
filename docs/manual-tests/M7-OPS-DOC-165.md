...
**You should see:** the builds finish without red error text, and Compose reports the containers as started. The migration step ends without an error. `cat VERSION` prints `0.7.35` or later. Write the number down.
...
2. Add the test folder. ☐
   ...
   **You should see:** the file accepted, with no red "Not added" note. Click **Library** in the rail. `harlow-lease.txt` is listed with the status **Ready**, not **Queued** or **Indexing**. Wait until it is.

3. Click **Ask** in the rail. Type `What is the monthly rent under the Harlow lease?` and send it. ☐

   **You should see:** an answer of 1,850 pounds, citing `harlow-lease.txt`. Under the question box there is a small line, `Askwell 0.7.35 · nothing leaves this machine`, with the number you wrote down.

4. Click **Memory** in the rail. Click **Add a fact**. A panel opens headed "Tell Askwell something before it asks." In the box that says `Subject, e.g. RFQ`, type `Harlow`. In the box below it, `What it means, e.g. Request for Quotation`, type `The Harlow Road unit, leased from Pemberton Estates`. Click **Add**. ☐
...
11. **Roll forward again**, to put the machine back as it was: ☐
    ...
    Reload the browser. **You should see:** `Askwell 0.7.35` under the question box (or the version you wrote down), and the same answer and memory card as before.
...
**Result, 2026-09-24, Linux (dev stack, `0.7.35`, from the terminal):** steps 1–4 and 11's migration pass; steps 5–10 **not run**. Step 1: §1's four commands, as written, gave `201`, then `"status": "done"`, then `200` and a 145 KB zip. Without the cookie, the same routes answer `401`. Steps 3–4: `db downgrade b5d09e3c71a8` printed one `Running downgrade c8f2a61d4b90 -> b5d09e3c71a8` line, and `db current` printed `b5d09e3c71a8`. Row counts for documents, chunks, memory and `audit_interactions` were `9/15/2/242` before, at the older revision, and after `db upgrade head`. `audit_decisions` went from 150 to 154 across the api/worker restart: startup records, not the downgrade. `askwell-verify` reported both chains intact. §1.2 step 4 as written: the good backup inspected `200` (`0.7.35`, 15 chunks), `POST /restore` refused the non-empty database with `409`, and a zip whose `manifest.json` is `{not json` answered `422 … is not a readable Askwell backup (JSONDecodeError)` from both routes (issue #714; before the fix it was a 500). Steps 5–10 switch the checkout to `67ab3d9d` and were not run in this session; they stay owned by #711.
...
3. **Make Askwell crash, twice**, once in each process. Put a file name and a question in the error, the way a real failure might: ☐
   ...
   **You should see:** for each, one `crash_report_written` log line naming the component, the `exception_type` and the report's path, followed by the usual traceback. The log line does **not** contain the message. That is the "a crash is logged locally" requirement.
   ...
   A third crash, in a background task nobody waits on, which is how a startup failure would look (§3 "When one is written"):
   ...
   async def fail():
       raise RuntimeError('discovery failed on harlow-lease.txt')
   async def main():
       install_loop_handler(load_settings(), 'api')
   ...
   **You should see:** one more `crash_report_written` line for `api`, then asyncio's own "Task exception was never retrieved" message. Step 4 then lists three reports, two of them `api`.

4. Back in the browser, reload the page. Click **Settings**, and scroll to **About → Report a problem → Crash reports**. ☐

   **You should see:** three lines, newest first, each like `2026-09-24 10:15 UTC, worker · Download` and `… UTC, api · Download`. The times are in UTC and within a minute of now. The worker's report is listed even though you are looking at the API. Both processes write to the same storage. While the list loads it reads "Reading…" for a moment. A red line "Askwell answered N listing crash reports." instead of the list is a defect.

5. Click **Download** on the `api` report. Open the downloaded file in a text editor. ☐

   **You should see:** readable JSON with exactly these keys, and nothing else:
   - `format`: `askwell-crash-report/1`
   - `askwell_version`: `0.7.35` (the number you wrote down)
   - `component`: `api`
   - `occurred_at`: today's date and a UTC time matching the list line
   - `platform`: `system`, `release`, `machine`, `python`
   - `profile`: a profile name such as `balanced`
   - `route`: `null` (this crash was not inside a request)
   - `exception`: `type` `builtins.RuntimeError`, and `chain` an empty list
   - `frames`: a list, each with `location`, `function` and `line`. The frame from the command you typed shows `location` `<other>`, never a path
   - `excluded`: a sentence ending "has not been sent anywhere."

   The words `could not open` from the error message appear **nowhere** in the file.

6. **Search the file for your material.** In the editor, search for each of `harlow`, `lease`, `rent`, `pemberton`, `settlement` as **whole words**. Or in a terminal: ☐

   ```
   grep -i -c -w "harlow\|lease\|rent\|pemberton\|settlement" ~/Downloads/crash-*-api-*.json
   ```

   **You should see:** no whole-word match in the editor, and `0` from `grep`. Whole words matter: the `platform` block has a `"release"` field, which contains `lease`, and a plain substring search reports it. Repeat for the `worker` report. Any match is a defect: the report would then carry the person's own material onto a public issue.

7. Scroll up to **Privacy and security → Network activity**. ☐

   **You should see:** the same **permitted** and **refused** numbers as in step 1. Nothing was sent. (If **Check for updates once a week** is ticked under **About → Update checking**, a scheduled check can move **permitted** by one on its own. Untick it before step 1 if it is ticked.)

8. **Without the interface.** §3 of the page gives a terminal way to get the reports. Follow it: ☐

   **You should see:** a folder containing the same three files.
...
**Result, 2026-09-24, Linux (dev stack, `0.7.35` rebuilt with this change; checked from the terminal rather than by clicking through):** pass. Step 3's three crashes each logged `crash_report_written` with component, type and path and no message. `GET /crash-reports` listed all three, newest first (`api`, `worker`, `api`). Each downloaded report held only the allow-listed fields, with `exception.type` `builtins.RuntimeError` / `ValueError` and an empty `chain`. The step 6 whole-word `grep` gave `0` for all three. The first run of this step used a substring `grep` and found `1` in each: the `"release"` field, not planted text. Step 6 now says whole words. Network counters read `refused 6, permitted 9` before and after. `podman compose cp` retrieved the same three files. Rerun steps 1–9 by clicking, as written, and replace this record.

**Rerun, 2026-09-24, Linux (dev stack, `0.7.35` reapplied onto `main` `98b42d9e`, terminal):** pass for the worker path. An uncaught `RuntimeError` whose message named `Harrington-settlement-draft.pdf` and a question wrote `crash-…-worker-….json`; `GET /crash-reports` listed it; the download held only the allow-listed fields, and a whole-word `grep -ciwE 'Harrington|settlement|agree|pay'` gave `0`. `/network` read `refused 6, permitted 9` before and after. `GET /crash-reports/..%2F..%2Frunning_version` answered `404`. The api-process and background-task crashes were not re-induced in this rerun; the record above stands for them.
...
Read `docs/rollback-and-incidents.md` §4 as if `0.7.35` had just broken ingestion on macOS. At each step, check that the page tells you what to do without asking anyone. ☐
...
- Who will find out that `0.7.35` is broken, and who will not? (§4.3: people with update checks on learn only that a *newer* version exists. Everyone else learns only from the repository. The page names issue #699 as making even that unreliable.)
