...
**Online AI, as built (`M8-ONLINE-SEC-169`).** Switching a conversation to online authorises one `host:port` for that conversation only, for at most `ASKWELL_ONLINE_AI_AUTHORISATION_TTL_SECONDS`. The proxy forwards to it only for a `CONNECT` carrying that conversation's credential (`Proxy-Authorization`), which lives in the API process alone. A local conversation, or a dependency, reaching the same host is refused. Disabling closes the grant at once, and an open tunnel is cut within a second. A restart of the API or of the proxy revokes every authorisation. Permitted connections are counted per conversation beside the total. The grant is authoritative, and `conversations.ai_backend` is reconciled to it, always towards local. The Redis entries are the persistent form, and restart revocation is what keeps that from outliving the scope. Reasoning: `decisions.md`, 2026-09-24.

**Who may write a grant (`M8-FIX-SEC-177`).** Every grant the proxy honours is a Redis key, so Redis runs with one user per service that connects and none for a connection that does not authenticate (`deploy/redis/users.acl`). The API is the only user that may write `askwell:egress:permitted_host`, `askwell:egress:grant:*` or `askwell:egress:conversation:*`. The proxy may read them, write its own counters, and `DEL` a conversation grant at startup — that one command on that one prefix, bound by an ACL selector. The worker holds the queue keys and nothing under `askwell:egress:`, so a document that exploits a parser cannot open a route out. `voice` and `inference-bridge` never open Redis and have no user. Passwords are in `.env` (C8); `deploy/redis/start.sh` renders their hashes into the ACL at every start. Reasoning: `decisions.md`, 2026-09-25.

**Online generation, as built (`M8-ONLINE-BE-170`).** In an online conversation, only the document-grounded answer's generation goes through that authorisation, to `askwell.inference.provider`, over the OpenAI-compatible chat stream the local server also speaks. Retrieval, abstention, citations and records do not move. The tool loop, SQL generation and the web-escalation answer stay local (#734). Every provider failure answers locally and names the reason. One provider request is one tunnel, so the proxy's count is a request count. Reasoning: `decisions.md`, 2026-09-24.

**Nothing is sent before it is described (`M8-ONLINE-FE-171`).** Being authorised is not enough to send. A send also needs this conversation to have confirmed the current pre-send disclosure, `askwell.online.DISCLOSURE`, recorded as an `online_ai_disclosure_confirmed` decision naming the conversation and the statement's version. Two places enforce it. `POST /ask` refuses an online conversation's question with `409` before recording anything, and `askwell.ask._online_client` builds no provider client, which covers voice and any other way into generation. While `DISCLOSURE` is `None` (#737), nothing can be confirmed, so nothing is sent. `POST /conversations` creates an empty local conversation, so it can be switched online before its first question. `GET /conversations/{id}/online` also reports `used_online` (from its own `online_ai_enabled` records), which keeps the marker on a conversation after its authorisation lapses. Reasoning: `decisions.md`, 2026-09-24.
...
- **The user's online provider key is one `settings` row, `online_provider_key`, encrypted under that same key** (`askwell.provider_key`, `M8-KEY-BE-173`). Beside it, in the clear, the provider it belongs to: the `host:port` a conversation's egress grant authorises and the model a request asks for. There is no destination or model setting in the environment; online AI is available only while a key is held and readable. Setting, changing or removing a passphrase re-encrypts it with the connection credentials. "Export everything" leaves the `settings` table out, and says so.
