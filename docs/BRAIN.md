...
**Not verified in this session.** The environment refused `podman`, `node`, `scripts/dev.sh` and `gh` alike, so nothing here has been executed. Owed: `scripts/dev.sh check` and `test` (four new API test modules, none ever run), `scripts/dev.sh test-db` (`api/tests/test_sources_records.py`, and the migration has never been applied to a database), `scripts/dev.sh web-check` and `web-build`, the cold-start walkthrough — add a PDF, add the identical file from another folder, confirm one document and both paths named — and the issues below, which could not be created.
