...
**Version:** `0.2.5` (see `VERSION`). Tickets bump `PATCH`; M0 landing took it to `0.2.0` (`AGENTS.md` §7).
...
**`M1-ADD-ING-025`** — background ingestion. It is the first thing that can move a document past `queued`: extraction, chunking, embedding, and the per-file progress the add screen currently and honestly does not show. It is also the first thing that can observe how long any of this takes, which is the estimate `M1-ADD-FE-022` deliberately refused to invent.
