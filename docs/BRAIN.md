...
**`M1-ADD-VAL-024`** — an unsupported file is refused by name, and a CSV is told when its turn comes. `0.2.4`.
...
**`M1-ADD-BE-023`** — source and document records with content-hash duplicate detection. It is what turns a queued batch into something that exists: the `sources` and `documents` rows, the decisions record for a source being added, and the **server-side** type check that the client's answer is not (see the open item below).
...
- **Needs an issue filing — a rejection is counted but never logged, so nothing durable records that Askwell turned a file away.** `M1-ADD-VAL-024` states "rejections are logged; they are not decisions records because nothing changed", and what exists is a `localStorage` counter. Nothing is sent to the API for a refused file — detection is entirely in the browser and the ticket adds no endpoint — so there is no operational-log line, and a user asking "why did my zip of contracts not appear" has only the screen they already closed. **Why it matters:** the requirement is real and will be read as satisfied because the ticket that stated it is closed. It also loses the one signal that would show a detection bug in the field: a corpus where 30% of files are being refused is a bug in `detect()`, and nobody can see that from a counter with no breakdown. **Where it surfaced:** `M1-ADD-VAL-024` audit requirements; `web/components/add/add-state.tsx`, the `REJECTED.add(refused)` call. **Options:** (a) `M1-ADD-BE-023` logs a refusal when it re-detects server-side — it is the first thing in the flow with a server round trip and the only place that will have re-checked the bytes itself; (b) add a small `POST /events/rejection` now, which is an endpoint whose only caller is a log line and which this ticket puts out of scope; (c) keep the counter and correct the ticket's audit line to say what is actually true. **Recommendation:** (a), carried forward as an explicit acceptance criterion of `M1-ADD-BE-023`. Labels: `phase:1`, `constraint:audit`.
