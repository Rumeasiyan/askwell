...
3. **Settled: the abstention surface's "Ask a larger model" offer names a provider key, never credits** (`../decisions.md`, 2026-09-23; `M7-FIX-FE-174`). With no key configured — true today, since M8 has not built anywhere to set one — it says the capability is not set up yet rather than a balance of zero. §3 above still describes the pre-2026-09-23 credit flow (balance, purchase, spending limit) and needs its own pass once M8's key-storage design lands; tracked separately (issue #658) so this ticket does not silently rewrite a section outside its stated scope.
