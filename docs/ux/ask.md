...
| **Online mode** | Persistent marker on the conversation, inside the composer's sticky band so it is visible without scrolling. The switch sits beside **Ask**, and there is no other place to turn online AI on. Before the first online send, exactly what will leave the machine, with a confirmation: the statement is `askwell.online.DISCLOSURE`, version `1`, approved 2026-09-25 (#737, #753) and shown as the server sends it. If it is ever unset, the disclosure says what is sent is not defined and the send is refused. In a conversation that has used online AI, each answer says *Answered locally* or *Answered online · model*. States: `../states-and-edge-cases.md` §1 (`M8-ONLINE-FE-171`) |
| **Provider refuses** — key rejected, account out of quota | Falls back to local, says so in the answer, continues. The conversation goes local and the marker names why and whose fix it is; switching back on is the user's choice. Never blocks — the product works offline for free. States: `../states-and-edge-cases.md` §1 (`M8-KEY-FE-175`) |
...
1. **Settled: suggested questions come from the corpus, not a model call** (`first-run.md` §6) — real filenames, real column names, assembled from what ingestion already extracted.
