...
| W1.2 | List the listening ports with the OS's own tool, not a repository script — the installed machine has none: `ss -ltn` on Linux, `lsof -nP -iTCP -sTCP:LISTEN` on macOS, `netstat -ano \| findstr LISTENING` on Windows | Every port Askwell opened is bound to `127.0.0.1` or `::1`. Nothing is bound to `0.0.0.0`, `::` or a LAN address. It is not on the network | | |
| W1.3 | **[net]** Read Settings → Network activity | Baseline recorded. On a clean machine, `permitted` is 0 | | |
...
| W2.5 | **[net]** Read Settings → Network activity | `permitted` unchanged | | |
...
| W4.5 | **[net]** Read Settings → Network activity | `permitted` unchanged | | |
...
| W5.9 | **[net]** Read Settings → Network activity | `permitted` unchanged, including after the live connection in W5.4 | | |
...
| W7.3 | **[net]** Read Settings → Network activity | `permitted` unchanged | | |
...
| # | Do | Expect | Status | Notes |
| - | -- | ------ | ------ | ----- |
| W8.1 | **[net]** Read Settings → Network activity | Record the count before the escalation | | |
...
| W8.4 | **[net]** Read Settings → Network activity | `permitted` moved only by the escalation you accepted, and by nothing else | | |
...
| W8.6 | **[net]** Read Settings → Network activity | `permitted` unchanged since W8.4 | | |
...
| W9.3 | Settings → About. Do **not** press **Check now**: it is the one request update checking makes, and this section is offline | The version matches `VERSION`. The licence, notices and security policy open in the page, with the network off. The support boundary is shown before the issue address. Update checking is off, and no newer-version line is shown. Under Report a problem, **Crash reports** reads "None.": nothing in W0–W8 made Askwell crash. A listed report is a `FAIL`; download it, attach it to the issue you file, and confirm it holds no file name or question from your corpus (`docs/rollback-and-incidents.md` §3) | | |
...
| W9.8 | In the directory the installer placed `compose.yaml` in (its `--prefix`. Unless you changed it: `~/.local/share/askwell/app` on Linux, `~/Library/Application Support/Askwell/app` on macOS, `%LOCALAPPDATA%\Askwell\app` on Windows), run `podman compose exec api askwell-verify`. Never run it from a development checkout: that checks the dev stack, not the install | Agrees with W9.7 | | |
| W9.9 | **[net]** Read Settings → Network activity | `permitted` is the W1.3 baseline plus W8's escalation only. Every `refused` entry is explained | | |
