...
| G8 | **Licence and notices** | `scripts/dev.sh notices` exits 0. The regenerated `NOTICES.md` is committed and unchanged on a second run. Everything bundled is GPLv3-compatible, permits redistribution and commercial use, and is not gated (C9) | `scripts/dev.sh notices`, then `git diff --exit-code NOTICES.md` | Release entry: the command's exit code, and the `NOTICES.md` commit |
