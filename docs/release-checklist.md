...
| - | ---- | -------------- | ------------- | ----------------- |
| G1 | **Version and changelog** | `VERSION` is the version being released. `CHANGELOG.md`'s top heading is that version. `api/tests/test_release_discipline.py` passes | `cat VERSION`; `head CHANGELOG.md` | Release entry: the version, and the commit SHA every other gate ran against |
...
row above. Record the verdict yourself.
