...
**Acceptance Criteria**
- **Acceptance Criteria:** Every release publishes a `SHA256SUMS` file covering every artefact, generated by a documented, scripted step rather than by hand. `docs/installing.md` is linked prominently — above the artefact list, not below it — from the release notes and the README. The release procedure names checksum generation and publication as its own documented step that someone who has not done it before can follow.
- **Edge Cases:** A partial artefact set (a platform's build failed) — that is a release blocker to raise, never published silently as if complete. An empty or missing artefact directory — the checksum step refuses rather than publishing an empty `SHA256SUMS`, which would be indistinguishable from "everything verified". A checksum that fails to verify after upload — the release is broken, not published as final, until re-uploaded and re-verified. A platform whose real installable bundle cannot yet be produced (issue #559) — that stays a named, tracked gap, not a workaround shipped quietly.
- **Permissions / Roles:** Single user — no roles in the product. Not applicable.
- **UI States:** `../ux/first-run.md` §2 — the first-run experience must not begin unexplained; `docs/installing.md` is the explanation, reached before install.
- **Validation Rules:** No signing credential is committed or implied (C8) — this ticket introduces none. `docs/installing.md`'s ordering (verification above bypass) may not be reversed by the release notes template or any future edit.
- **Audit / Logging Requirements:** None inside the product — checksums are a release-time artefact, not application state.
- **Analytics Events:** None.

**Real-World Example Scenarios**
- A lawyer downloads Askwell on a Mac, reads `docs/installing.md` before the security warning would otherwise make them close the tab, verifies the checksum, and goes through **Open Anyway** knowing the file matches what was published.
- A release is prepared on a Friday and nobody publishes it until `SHA256SUMS` has been generated, uploaded, and re-verified by downloading it back — the step this ticket makes routine rather than something remembered under time pressure.

**Dependencies & Assumptions**
- **Dependencies:** M7-TAURI-DEPLOY-181, M7-PACK-DEPLOY-139, M7-PACK-DEPLOY-140, M7-PACK-DEPLOY-141.
- **API / Data Touchpoints:** None inside the product. The release publishing step (GitHub Releases).
- **Assumptions:** A real installable bundle per platform can be assembled by the time of release — today that is the trimmed-tarball layout `deploy/*/install.sh` already expects, since no packaging pipeline produces anything else yet (issue #559). This ticket does not build that pipeline; it checksums and publishes whatever `#559` (or manual assembly, until then) produces.

**Testing Notes / Scenarios**
- **Checksum round-trip:** Run `scripts/release-checksums.sh` against a directory of artefacts, confirm `SHA256SUMS` is written and verifies (`sha256sum -c` / `shasum -a 256 -c`), and confirm it refuses an empty or missing directory rather than publishing an empty file.
- **Cold-start manual walkthrough:** Take a freshly downloaded artefact for each platform to a machine that has never seen Askwell and follow `docs/installing.md` exactly as written, checksum first. Tracked per platform: `docs/manual-tests/M7-PACK-DEPLOY-139.md` (Linux), `M7-PACK-DEPLOY-140.md` (Windows, issue #590 — not yet run for real), `M7-PACK-DEPLOY-141.md` (macOS, issue #592 — not yet run for real).
- **Known gaps:** Real cross-platform cold-start verification is blocked on #559 (no real artefact yet) and on #590/#592 (no test hardware yet for Windows/macOS) — tracked, not silently skipped. No app store distribution. Update delivery is blocked and will need this again.

**Effort & Granularity Check**
- **Estimate:** 4–6 hours · **Priority:** Critical
- **Labels / Component:** `phase:6`, deployment, security
- **Granularity:** Checksum generation, the release procedure, and the notes template — one documented path across three platforms' artefacts, not the packaging pipeline that produces them (#559, separately tracked).
