...
C9 changed on 2026-09-25, by decision (`docs/decisions.md`, "Askwell is relicensed to GPLv3"; `M8-FIX-DOC-179`): it used to require only that a bundled *model* permit redistribution under Apache-2.0, and now requires everything bundled to be GPLv3-compatible. The test is compatibility with GPLv3, not permissiveness — a GPL-2.0-only dependency is as much a blocker as a non-commercial model.
...
| C9 | **Everything bundled — model, weight, traineddata or dependency — must be GPLv3-compatible, must permit redistribution and commercial use, and must not be access-gated.** Verified against the registry before the name is written into configuration. GPL-2.0-only, AGPL, SSPL, non-commercial, no-derivatives and unlicensed terms cannot ship; a licence that could be either side of that line (a bare "GPL") is decided by a person, not assumed. | Askwell ships weights and dependencies inside a redistributable offline installer under GPL-3.0-or-later, relicensed from Apache-2.0 on 2026-09-25 so that spoken answers ship (`docs/decisions.md`). A component whose terms GPLv3 cannot carry cannot ship however well it performs — and an installer cannot click through an access agreement. Discovering this at packaging time in Phase 7 would be phase-blocking. | Model selection; `scripts/dev.sh notices`, the release licence gate (`api/src/askwell/notices.py`); the offline bundle build (`docs/build-plan.md` Phase 7) |
