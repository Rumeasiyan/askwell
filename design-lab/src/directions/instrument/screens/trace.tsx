...
function Step({ n, title, ms, detail, expand }: any) {
  return (
    <div className="grid grid-cols-[22px_minmax(0,1fr)_auto] items-baseline gap-x-3 border-b border-[var(--rule)] py-3 last:border-0">
      <span className="text-[12px] tabular-nums text-[var(--muted)]">{n}</span>
      <span className="text-[13px]">{title}</span>
      <span className="text-[12px] tabular-nums text-[var(--muted)]">{ms}</span>
      <span />
      <div className="col-span-2 mt-1 flex flex-col gap-1">
        <span className="text-[12.5px] text-[var(--muted)]">{detail}</span>
        {expand && <span className="text-[12px] text-[var(--provenance)]">▸ {expand}</span>}
      </div>
    </div>
  )
}
        ...
        <Main>
          ...
          <div className="rounded-[var(--radius)] border border-[var(--rule)] bg-[var(--surface)] px-4">
            ...
            <Step n="3" title="Looked up schema" ms="40 ms"
              detail={<>invoices · used your note on <span className="text-[var(--provenance)]">st_cd</span></>}
              expand="show what the model was given" />
            <Step n="4" title="Queried sales-2024" ms="240 ms"
              detail={<>7 rows · <span className="text-[var(--inferred)]">LIMIT 1000 added by Askwell</span> · validation passed</>}
              expand="show query and validation" />
            <Step n="5" title="Wrote the answer" ms="8.2 s" detail="3 claims, all cited" />
          </div>

          <div className="rounded-[var(--radius)] border border-[var(--rule)] bg-[var(--sunk)] p-4">
            ...
            <div className="mt-2 flex flex-col gap-2">
              {[['0.81', 'supplier-agreement-2024.pdf p.14', true],
                ...
                <div key={f} className="flex items-baseline gap-3 text-[12.5px]">
                  <span className={`w-10 tabular-nums ${used ? 'text-[var(--provenance)]' : 'text-[var(--muted)]'}`}>{s}</span>
                  <span className={used ? '' : 'text-[var(--muted)]'}>{f}</span>
                  <span className="ml-auto text-[11px] text-[var(--muted)]">{used ? 'used' : 'below threshold'}</span>
                </div>
              ))}
            </div>
          </div>
